...
| [**Chain of Responsibility**](./chain-of-responsiblity/readme) | Passes a request along a chain of handlers, where each handler can either handle the request or pass it along the chain.                                 | Used for handling requests with multiple possible handlers, such as in middleware systems.                                   |
